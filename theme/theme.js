/*
  Theme Object. Common format used then when styling.
*/
export const theme = {
  fonts: {
    main: "Space Grotesk,sans-serif",
  },
  colors: {
    primary: "rgba(255 255 255)",
    paragraph: "rgba(255 255 255 / 0.5)",
    links: "rgba(255 255 255 / 0.75)",
    button3: "rgba(29 222 255 / 1)",
    description: "rgba(26 39 64 / 0.9)",
  },
  background: {
    bg1: "rgba(15 22 36 / 1)",
    nav: "rgba(15 22 36 / 0.85)",
  },
  breakpoints: {
    break1330: "(max-width: 1330px)",
    break1280: "(max-width: 1280px)",
    break1196: "(max-width: 1196px)",
    break1081: "(max-width: 1081px)",
    break997: "(max-width: 997px)",
    break931: "(max-width: 931px)",
    break864: "(max-width: 864px)",
    break800: "(max-width: 800px)",
    break768: "(max-width: 768px)",
    break700: "(max-width: 700px)",
    break650: "(max-width: 650px)",
    break621: "(max-width: 621px)",
    break585: "(max-width: 585px)",
    break570: "(max-width: 570px)",
    break504: "(max-width: 504px)",
    break460: "(max-width: 460px)",
    break422: "(max-width: 422px)",
    break400: "(max-width: 400px)",
    break339: "(max-width: 339px)",
  },
};
